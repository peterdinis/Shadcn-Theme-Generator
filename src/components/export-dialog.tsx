...
import { Copy, Check, Info, Zap, FileCode } from "lucide-react";
...
export function ExportDialog() {
  ...
  const highlightCode = (code: string) => {
    return code.split('\n').map((line, i) => {
      if (line.includes('/*')) return <span key={i} className="text-zinc-500 italic">{line}{'\n'}</span>;
      if (line.includes(': ')) {
      ...
      if (line.includes('@theme') || line.includes('@layer') || line.includes('@variant')) {
        return <span key={i} className="text-purple-400 font-bold">{line}{'\n'}</span>;
      }
      ...
      <DialogContent className="max-w-4xl p-0 gap-0 overflow-hidden bg-zinc-950/90 backdrop-blur-3xl border border-white/10 shadow-[0_0_50px_-12px_rgba(0,0,0,1)] rounded-[32px]">
          ...
          <div className="flex items-end justify-between">
            <div className="space-y-3">
              <div className="inline-flex items-center gap-2 px-2.5 py-1 rounded-full bg-primary/10 border border-primary/20">
                ...
                <span className="text-[10px] font-black uppercase tracking-[0.2em] text-primary">Version {version.toUpperCase()}</span>
              </div>
              ...
              <DialogDescription className="text-base text-zinc-400 font-medium max-w-md leading-relaxed">
                Your production-ready CSS variables are ready. Optimized for performance and readability.
              </DialogDescription>
            </div>
            ...
            <div className="flex h-11 p-1 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md">
              <button 
                ...
                className={cn(
                  ...
                  version === "v4" ? "bg-white text-black shadow-xl scale-[1.02]" : "text-zinc-500 hover:text-zinc-300"
                )}
              >
                v4
              </button>
              <button 
                ...
                className={cn(
                  "px-6 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all duration-300",
                  version === "v3" ? "bg-white text-black shadow-xl scale-[1.02]" : "text-zinc-500 hover:text-zinc-300"
                )}
              >
                v3
              </button>
          ...
          <div className="relative group">
            ...
            <div className="flex items-center justify-between px-6 py-4 rounded-t-[24px] bg-white/5 border-x border-t border-white/10 border-b-0 backdrop-blur-xl">
              ...
              <div className="flex items-center gap-2 px-3 py-1 rounded-lg bg-black/40 ring-1 ring-white/5">
                ...
                <span className="text-[10px] font-mono text-zinc-400 tracking-wider">globals.css</span>
              </div>
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="h-16 flex flex-col justify-center px-8 rounded-[24px] bg-white/5 border border-white/10 shrink-0">
               <div className="flex items-center gap-3">
                 ...
                 <span className="text-[11px] font-black uppercase tracking-widest text-zinc-400">System Ready</span>
               </div>
               <span className="text-[10px] text-zinc-600 font-bold">SHA: {Math.random().toString(36).substring(7).toUpperCase()}</span>
            </div>
          </div>
        </div>
        ...
        <div className="px-10 py-5 bg-white/[0.02] border-t border-white/5 flex items-center justify-between">
          <div className="flex items-center gap-4">
             ...
             <span className="text-[10px] font-bold text-zinc-500 tracking-wide">Used by 1,240+ developers</span>
          </div>
          <div className="flex items-center gap-6">
             <div className="flex items-center gap-2">
               <div className="w-1 h-1 rounded-full bg-zinc-700" />
               <span className="text-[9px] font-black text-zinc-600 uppercase">Optimized</span>
             </div>
             <div className="flex items-center gap-2">
               <div className="w-1 h-1 rounded-full bg-zinc-700" />
               <span className="text-[9px] font-black text-zinc-600 uppercase">Verified</span>
             </div>
          </div>
        </div>
      </DialogContent>
